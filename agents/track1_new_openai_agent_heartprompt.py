...
with (curr_dir / 'merchat_func_copy.json').open('r', encoding='utf-8') as f:
    merchat = json.load(f)

with (curr_dir / 'guild_func_copy.json').open('r', encoding='utf-8') as f:
    guild = json.load(f)

    
class NewOpenAIAgent_heart(object):
    """
    ...
    def generate_functions_and_responses(self, tool_registry, action_registry, worldview, persona, role, knowledge, state, dialogue, executor): 
        ...
        career_function = []
        if persona['occupation'] == "Merchant who sells weapons for adventurers.":
            career_function = merchat
            
        elif persona['occupation'] == "Guild Receptionist.":
            career_function = guild
      
       
        response = self.client.chat.completions.create(
          model="gpt-4o-mini",
          messages=function_messages,
          functions=career_function,
          function_call="auto"
        )
    ...
    def _create_messages_for_function(self, tool_registry, action_registry, dialogue, role):
      ...
      function_prompt = (
          ...
          "You are provided with function definitions : \n"
          "#Always integrate relevant context from previous turns when determining arguments for a function call.\n"
          "# Role: {role_npc}\n\n"
          "# Chat History:\n"
          "{dialogue}"
      )
