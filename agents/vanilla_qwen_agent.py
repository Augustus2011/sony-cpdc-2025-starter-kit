...
class VanillaQwenAgent(object):
    """
        VanillaLlamaAgent modified for Qwen/Qwen2.5-7B-Instruct-1M model for the Sony CPDC 2025 Challenge. 

        This agent takes in information from the dialogue, the functions, and the background of the scenario, 
        It calls a Qwen2.5-7B-Instruct-1M model twice per turn. 
        The first call aims to find appropriate functions to call, 
        and the second generates responses. 

        Attributes: 
            model: The Qwen2.5-7B-Instruct-1M model. 
            tokenizer: The tokenizer for the Qwen2.5-7B-Instruct-1M model. 
    ...
    def __init__(self):
        ...
        # Qwen tokenizer typically has pad_token set correctly, but let's ensure it
        if self.tokenizer.pad_token_id is None:
            self.tokenizer.pad_token_id = self.tokenizer.eos_token_id

        # Qwen models use standard eos_token_id, no special terminators needed
        self.terminators = [self.tokenizer.eos_token_id]

        self.max_seq_len = 2048
        self.max_gen_len = 50 #2048
    ...
    def generate_functions_and_responses(self, tool_registry, action_registry, worldview, persona, role, knowledge, state, dialogue, executor):
        ...
        2. Call the model (Qwen2.5-7B-Instruct-1M) to generate the necessary function calls. 
        ...
        5. Call the model (Qwen2.5-7B-Instruct-1M) to generate the text response. 
        ...
        # Generate with Qwen-style parameters
        ...
        res_str = self.tokenizer.decode(generated_ids[0], skip_special_tokens=True)
        items = res_str.split("\n")
        ...
        # Apply chat template and prepare inputs for dialogue generation
        text = self.tokenizer.apply_chat_template(
            dialogue_messages,
            tokenize=False,
            add_generation_prompt=True
        ...
        # Generate response with Qwen-style parameters
        generated_ids = self.model.generate(
            **model_inputs,
            max_new_tokens=self.max_gen_len,
            do_sample=False,
        ...
        res_str = self.tokenizer.decode(generated_ids[0], skip_special_tokens=True).replace("\n", " ")
    ...
    def _create_messages_for_function(self, tool_functions, action_functions, dialogue):
        ...
        messages = []
        # Adding Qwen system message format
    ...
    def _create_messages_for_dialogue(self, worldview, persona, role, knowledge, state, dialogue, function_results):
        ...
        creates the messages to feed to the model to generate the text response. 
        ...
        dialogue_prompt = (
            ...
            "\n"
            "# Guide word using in the dialogue\n"
